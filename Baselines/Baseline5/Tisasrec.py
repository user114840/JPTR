...
class TimeAwareMultiHeadAttention(torch.nn.Module):
  ...
  # time_mask: mask padding items; attn_mask: causal mask (lower triangle)
  ...
  # 1.unsqueeze(): time_mask (batch_size,maxlen) -> (batch_size,maxlen,1); fill last dim with 1
  # 2.repeat(): (batch_size,maxlen,1) -> (head_num*batch_size,maxlen,1); scale first dim
        time_mask = time_mask.expand(-1, -1, attn_weights.shape[-1])

  # tips: attn_weights=(B,maxlen,maxlen); each row is an item attention vector
        ...
        attn_weights = self.softmax(attn_weights)   # Eq. (7)
...
class TiSASRec(torch.nn.Module): # similar to torch.nn.MultiheadAttention
    def __init__(self, user_num, item_num, args):
        ...
        self.item_emb = torch.nn.Embedding(self.item_num+1, args.hidden_units, padding_idx=0)   # Eq. (3)
        self.item_emb_dropout = torch.nn.Dropout(p=args.dropout_rate)

        self.abs_pos_K_emb = torch.nn.Embedding(args.maxlen, args.hidden_units)                 # Eq. (3)
        self.abs_pos_V_emb = torch.nn.Embedding(args.maxlen, args.hidden_units)                 # Eq. (4)
        self.time_matrix_K_emb = torch.nn.Embedding(args.time_span+1, args.hidden_units)        # Eq. (5)
        self.time_matrix_V_emb = torch.nn.Embedding(args.time_span+1, args.hidden_units)        # Eq. (5)
    ...
    def seq2feats(self, user_ids, log_seqs, time_matrices):
        ...
        if isinstance(time_matrices, torch.Tensor):
            time_matrices = time_matrices.to(self.dev)
        else:
        ...
        for i in range(len(self.attention_layers)):
            ...
            mha_outputs = self.attention_layers[i](Q, seqs,
                                            ...
                                            abs_pos_K, abs_pos_V)   # output after time multi-head self-attention
    ...
    def forward(self, user_ids, log_seqs, time_matrices, pos_seqs, neg_seqs): # for training
        ...
        pos_logits = (log_feats * pos_embs).sum(dim=-1)  # positive logits
        neg_logits = (log_feats * neg_embs).sum(dim=-1)  # negative logits
